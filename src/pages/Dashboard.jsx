...
function ReferralBlock({ profile }) {
  ...
  const shareText = refLink
    ? `Hola! 👋 Te invito a Equia, la plataforma que conecta startups con talento vía IA.\n\nUsá mi código *${profile.referral_code}* al registrarte y recibís 50 créditos extra para arrancar.\n\n👉 ${refLink}`
    : ''
  ...
  return (
    <div style={{ marginBottom: 36, padding: '22px 24px', border: '1px solid #e8e8e8', borderRadius: 14, background: '#fafafa' }}>
      ...
      {/* Link row */}
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', background: '#fff', border: '1px solid #e0e0e0', borderRadius: 9, padding: '10px 14px', marginBottom: 10 }}>
        <span style={{ flex: 1, fontSize: 12, color: '#666', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{refLink}</span>
        <button
          onClick={copyLink}
          style={{ flexShrink: 0, padding: '7px 14px', background: copied ? '#22c55e' : '#E8611A', border: 'none', borderRadius: 7, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter,sans-serif', transition: 'background .2s' }}
        >
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 16px', background: '#25D366', border: 'none', borderRadius: 8, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter,sans-serif', transition: 'opacity .15s' }}
        ...
        <button
          onClick={copyMsg}
          style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 16px', background: copiedMsg ? '#22c55e' : '#fff', border: '1px solid #e0e0e0', borderRadius: 8, color: copiedMsg ? '#fff' : '#555', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter,sans-serif', transition: 'all .2s' }}
        >
          {copiedMsg ? '✓ Mensaje copiado' : '📋 Copiar mensaje completo'}
        </button>
      </div>

      {/* Preview text */}
      <div style={{ marginTop: 12, padding: '12px 14px', background: '#fff', border: '1px solid #e8e8e8', borderRadius: 8 }}>
        <div style={{ fontSize: 11, color: '#aaa', fontWeight: 600, marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1 }}>Vista previa del mensaje</div>
        <pre style={{ fontSize: 12, color: '#666', lineHeight: 1.65, whiteSpace: 'pre-wrap', margin: 0, fontFamily: 'Inter, sans-serif' }}>{shareText}</pre>
      </div>
    </div>
  )
}
