...
export default function TalentCard({ talent, onInvite, showInvite = false }) {
  ...
  return (
    <div
      ...
      style={{ padding: 20, background: '#000', borderBottom: '1px solid #1a1a1a', transition: 'background .15s', cursor: 'pointer' }}
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {talent.location && <div style={{ fontSize: 12, color: '#444', marginTop: 2 }}>📍 {talent.location}</div>}
        </div>
      ...
      {talent.bio && (
        <p style={{ fontSize: 13, color: '#555', lineHeight: 1.6, marginBottom: 12, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{talent.bio}</p>
      )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 12, borderTop: '1px solid #1a1a1a' }}>
        <span style={{ fontSize: 12, color: '#555' }}>{talent.projects} proyectos</span>
        <div style={{ display: 'flex', gap: 7 }} onClick={e => e.stopPropagation()}>
          ...
          <button
            onClick={() => navigate(`/talento/${talent.id}`)}
            style={{ padding: '7px 14px', fontSize: 12, fontWeight: 600, background: 'none', border: '1px solid #222', color: '#fff', borderRadius: 6, cursor: 'pointer', fontFamily: 'Inter, sans-serif', transition: 'all .15s' }}
